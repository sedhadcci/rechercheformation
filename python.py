...
def search_data(df, query, column='Compétences'):
    ...
    for keyword in keywords:
        df_copy['Match_Score'] = df_copy[column].apply(lambda row: process.extractOne(keyword, [str(row)], score_cutoff=70))
...
if uploaded_file is not None:
    ...
    query = st.text_input("Entrer le(s) mot(s) clé(s) séparés par des virgules")
    if query:
